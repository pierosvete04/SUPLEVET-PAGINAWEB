...
interface SelectorRegaloBandanasProps {
  /** Subtotal del pedido — decide si califica un regalo por monto mínimo. */
  subtotal: number;
  /** Unidades de combo en el pedido — un slot de bandana por cada una. */
  combosQty: number;
  selecciones: (BandanaSeleccion | null)[];
  onCambiarSlot: (indice: number, seleccion: BandanaSeleccion | null) => void;
  /** "carrito": colapsable, con progreso/mensaje corto (drawer y /carrito).
   * "checkout": siempre expandido, como paso propio del checkout o del panel. */
  variant?: "carrito" | "checkout";
  /** "admin": lo llena el equipo sobre un pedido cargado a mano, no el
   * comprador — solo cambia la redacción. */
...
// Selector de bandanas de regalo — vive en el carrito (drawer, /carrito), como
// paso propio del checkout y en /admin/pedidos/nuevo. Soporta N slots: uno por
// cada combo comprado si el regalo activo es de categoría ("cualquier combo"),
// o uno solo si es de monto mínimo / evento (comportamiento histórico). Cada
// slot elige diseño + talla (S, M o L) de forma independiente.
//
// No conoce el carrito a propósito: recibe subtotal/combos por props para que
// el panel de administración pueda armar el mismo regalo sobre un pedido que
// se está creando a mano, con exactamente las mismas reglas que la web.
export function SelectorRegaloBandanas({
  subtotal,
  combosQty,
  selecciones,
  onCambiarSlot,
    ...
    <div className="rounded-[17px] border border-border bg-soft-gray p-4">
      ...
      {!calificaParaRegalo && esCategoria && (
        <p className="mt-1 font-body text-xs text-secondary">
          {esAdmin ? "Agrega un combo al pedido" : "Agrega cualquier combo a tu carrito"} para
          desbloquear la bandana gratis: <strong>{regalo.nombre}</strong>
        </p>
      )}
        ...
        <div className="mt-3 flex flex-col gap-4">
          {Array.from({ length: slotsCount }, (_, i) => i).map((slot) => (
            <SlotBandana
              key={slot}
              etiqueta={
                slotsCount > 1 ? `Bandana #${slot + 1}` : esAdmin ? "Bandana del pedido" : "Tu bandana"
              }
              disenos={disenos}
              seleccion={selecciones[slot] ?? null}
              onCambiar={(seleccion) => onCambiarSlot(slot, seleccion)}
            />
          ))}
        </div>
      )}
